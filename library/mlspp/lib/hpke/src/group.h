...
// TriCord-authored replacement for upstream mlspp's lib/hpke/src/group.h.
// The abstract Group interface below is reproduced verbatim from upstream
// (BSD-2-Clause, Cisco Systems) so dhkem.cpp (vendored unchanged) keeps
// compiling against the same shape. The upstream EVPGroup subclass is NOT
// reproduced here -- it bound Group directly to OpenSSL's EVP_PKEY, which this
// backend (mbedtls_group.cpp) does not use. mbedtls_group.cpp implements Group
// directly instead of going through an intermediate EVP-shaped base class.
